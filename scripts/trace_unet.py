...
class DDIMSampler(nn.Module):
    ...
    @autocast('cuda')
    @torch.no_grad()
    def forward(self, img, condition, unconditional_condition, t, t_prev):
        ...
        img = self.model.q_sample(pred_x0, t_prev, e_t)
        return img
...
def main(opt):
    ...
    with torch.no_grad():
        ...
        scripted_sampler = torch.jit.trace(sampler, (img_in, context, uncondition_context, t_in, t_prev))
        ...
        for index, i in enumerate(reversed(ddim_timesteps)):
            ...
            img = scripted_sampler(img, condition, unconditional_condition, t, t_prev)
            # x_in = torch.cat([img] * 2)
            # t_in = torch.cat([t] * 2)
            # condition_in = torch.cat([unconditional_condition, condition])
        ...
        decoded_images = sampler.model.decode_first_stage(img)
